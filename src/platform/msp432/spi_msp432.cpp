...
spi_msp432::spi_msp432(EUSCI_A_SPI_Type *mod, gpio_pin & cs, const bool spi_master,
					   uint16_t mode)
:	_master(spi_master),
...
spi_msp432::spi_msp432(EUSCI_B_SPI_Type *mod, gpio_pin & cs, const bool spi_master,
					   uint16_t mode)
:   _master(spi_master),
    _EUSCI_CTLW0(mod->CTLW0), _EUSCI_BRW(mod->BRW),
	_EUSCI_STATW(mod->STATW), _EUSCI_RXBUF(mod->RXBUF),
