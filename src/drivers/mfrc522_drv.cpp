 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void mfrc522_drv::PCD_WriteRegister(PCD_Register reg,   ///< The register to write to. One of the PCD_Register enums.
 ...
 * The interface is described in the datasheet section 8.1.2.
 */
void mfrc522_drv::PCD_ReadRegister(PCD_Register reg,    ///< The register to read from. One of the PCD_Register enums.
 ...
 */
void mfrc522_drv::PCD_SetRegisterBitMask(PCD_Register reg,    ///< The register to update. One of the PCD_Register enums.
 ...
 */
void mfrc522_drv::PCD_ClearRegisterBitMask(PCD_Register reg,    ///< The register to update. One of the PCD_Register enums.
...
mfrc522_drv::PCD_TransceiveData(uint8_t *sendData,        ///< Pointer to the data to transfer to the FIFO.
 ...
 */
mfrc522_drv::StatusCode
mfrc522_drv::PCD_CommunicateWithPICC(uint8_t command,        ///< The command to execute. One of the PCD_Command enums.
 ...
 */
mfrc522_drv::StatusCode
mfrc522_drv::PICC_RequestA(uint8_t *bufferATQA,    ///< The buffer to store the ATQA (Answer to request) in
 ...
 */
mfrc522_drv::StatusCode
mfrc522_drv::PICC_WakeupA(uint8_t *bufferATQA,    ///< The buffer to store the ATQA (Answer to request) in
                      uint8_t *bufferSize    ///< Buffer size, at least two uint8_ts. Also number of uint8_ts returned if STATUS_OK.
) {
 ...
 */
mfrc522_drv::StatusCode
mfrc522_drv::PICC_REQA_or_WUPA(uint8_t command,        ///< The command to send - PICC_CMD_REQA or PICC_CMD_WUPA
 ...
 */
mfrc522_drv::StatusCode
mfrc522_drv::PCD_Authenticate(uint8_t command,        ///< PICC_CMD_MF_AUTH_KEY_A or PICC_CMD_MF_AUTH_KEY_B
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
mfrc522_drv::StatusCode mfrc522_drv::MIFARE_Ultralight_Write(uint8_t page,        ///< The page (2-15) to write to.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
mfrc522_drv::StatusCode mfrc522_drv::MIFARE_Decrement(uint8_t blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
mfrc522_drv::StatusCode mfrc522_drv::MIFARE_Increment(uint8_t blockAddr, ///< The block (0-0xff) number.
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
mfrc522_drv::StatusCode mfrc522_drv::MIFARE_Restore(uint8_t blockAddr ///< The block (0-0xff) number.
) {
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
mfrc522_drv::StatusCode mfrc522_drv::MIFARE_TwoStepHelper(uint8_t command,    ///< The command to use
 ...
 * @return STATUS_OK on success, STATUS_??? otherwise.
 */
mfrc522_drv::StatusCode mfrc522_drv::MIFARE_Transfer(uint8_t blockAddr ///< The block (0-0xff) number.
) {
    mfrc522_drv::StatusCode result;
    uint8_t cmdBuffer[2]; // We only need room for 2 uint8_ts.
 ...
 */
const char *mfrc522_drv::GetStatusCodeName(mfrc522_drv::StatusCode code    ///< One of the StatusCode enums.
) {
    switch (code) {
        case STATUS_OK:
            return ("Success.");
 ...
 * @return PICC_Type
 */
mfrc522_drv::PICC_Type mfrc522_drv::PICC_GetType(uint8_t sak        ///< The SAK uint8_t returned from PICC_Select().
) {
 ...
 * @return const __FlashStringHelper *
 */
const char *mfrc522_drv::PICC_GetTypeName(PICC_Type piccType    ///< One of the PICC_Type enums.
) {
    switch (piccType) {
 ...
 */
void mfrc522_drv::PICC_DumpToSerial(Uid *uid    ///< Pointer to Uid struct returned from a successful PICC_Select().
) {
    MIFARE_Key key;
 ...
 */
void mfrc522_drv::PICC_DumpDetailsToSerial(Uid *uid) {    ///< Pointer to Uid struct returned from a successful PICC_Select().
    // UID
    printf("Card UID:");
