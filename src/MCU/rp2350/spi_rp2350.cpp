...
// ---------------------------------------------
//
// SPI driver for RP2350. It support master
...
spi_rp2350::~spi_rp2350() {
    ...
    // Reset UART
    if (_index)  RESETS_SET.RESET.SPI1 <<= 1;
    else         RESETS_SET.RESET.SPI0 <<= 1;
    // De-configure the digital RX/TX lines
    _miso.setSEL( GPIO_CTRL_FUNCSEL__null );
    _mosi.setSEL( GPIO_CTRL_FUNCSEL__null );
    _sclk.setSEL( GPIO_CTRL_FUNCSEL__null );
}

void spi_rp2350::init() {
...
int16_t spi_rp2350::spiTxRx(const uint8_t *txbuf, uint8_t *rxbuf, uint16_t len)
{
    if (!_init) init();
    if (_master) {
        // Activate CS line
...
int16_t spi_rp2350::spiTx(const uint8_t *txbuf, uint16_t len) {
    if (!_init) init();
    if (_master) {
        // Activate CS line
        if (_generate_CS) _cs.gpioWrite(LOW);
...
int16_t spi_rp2350::spiRx(uint8_t tx_byte, uint8_t *rxbuf, uint16_t len) {
    if (!_init) init();
    if (_master) {
        // Activate CS line
        if (_generate_CS) _cs.gpioWrite(LOW);
...
void SPI0_IRQ_Handler(void) {
    spi_rp2350::_intHandler[0]( SPI0.SSPDR );
}

void SPI1_IRQ_Handler(void) {
    spi_rp2350::_intHandler[1]( SPI1.SSPDR );
}
