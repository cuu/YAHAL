...
void ws2812_rp2350::LED::on() {
    _color = _on_color;
    _ws2812_rp2350->update(_index);
}

void ws2812_rp2350::LED::off() {
    _color = 0;
    _ws2812_rp2350->update(_index);
}

void ws2812_rp2350::LED::toggle() {
    _color = _color ? 0 : _on_color;
    _ws2812_rp2350->update(_index);
}

bool ws2812_rp2350::LED::is_on() {
    return _color;
}

void ws2812_rp2350::LED::set_color(uint32_t rgb) {
    _color = _ws2812_rp2350->xRGB_to_GRBx(rgb);
    _ws2812_rp2350->update(_index);
}

void ws2812_rp2350::LED::set_on_color(uint32_t rgb) {
    _on_color = _ws2812_rp2350->xRGB_to_GRBx(rgb);
}

// ws2812_rp2040 implementation
///////////////////////////////
ws2812_rp2350::ws2812_rp2350(gpio_pin_t pin,
                             uint16_t   size)
: _gpio(pin), _size(size) {
    // Allocate LED array and set base class pointers
    _leds = new LED[_size];
    for(uint16_t i=0; i < _size; ++i) {
        _leds[i]._ws2812_rp2350 = this;
        _leds[i]._index = i;
    }
}

ws2812_rp2350::~ws2812_rp2350() {
    delete [] _leds;
    _leds = nullptr;
    _gpio.setSEL(GPIO_CTRL_FUNCSEL__null);
}

void ws2812_rp2350::init() {
    // configure GPIO pin to use PIO0 output
    _gpio.setSEL(GPIO_CTRL_FUNCSEL__pio0);
    // Set up the PIO state machine
    _sm = pio_rp2350::pio0.loadProgram(ws2812_program);
...
void ws2812_rp2350::set_colors(uint32_t * values, uint16_t size) {
    assert(size <= _size);
...
uint32_t ws2812_rp2350::xRGB_to_GRBx(uint32_t rgb) {
...
void ws2812_rp2350::update(uint16_t index) {
    if (!_init) init();
