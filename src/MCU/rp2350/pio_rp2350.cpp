...
void SM::attachTXNFULLIrq(function<void()> handler) {
    if (pio_index)
        ...
        pio_rp2350::_handler_pio0[sm_index + 4] = handler;
}
...
void SM::attachRXNEMPTYIrq(function<void()> handler) {
    if (pio_index)
        ...
        pio_rp2350::_handler_pio0[sm_index + 0] = handler;
}
...
pio_rp2350::pio_rp2350(PIO0_t & pio, uint8_t pio_index)
: _pio(pio),
...
pio_rp2350::~pio_rp2350() {
    // Disable all state machines
    _pio.CTRL.SM_ENABLE  = 0x0;
}

SM* pio_rp2350::loadProgram(const pio_program & prgm) {
...
extern "C" {
    void PIO0_IRQ_0_Handler(void) {
        ...
        while (uint8_t pos = __builtin_ffs(status)) {
            ...
            if (pio_rp2350::_handler_pio0[pos])
                pio_rp2350::_handler_pio0[pos]();
        }
    }
    ...
    void PIO1_IRQ_0_Handler(void) {
        ...
        while (uint8_t pos = __builtin_ffs(status)) {
            ...
            if (pio_rp2350::_handler_pio1[pos])
                pio_rp2350::_handler_pio1[pos]();
        }
    }
    ...
    void PIO2_IRQ_0_Handler(void) {
        ...
        while (uint8_t pos = __builtin_ffs(status)) {
            ...
            if (pio_rp2350::_handler_pio2[pos])
                pio_rp2350::_handler_pio2[pos]();
        }
    }
}
