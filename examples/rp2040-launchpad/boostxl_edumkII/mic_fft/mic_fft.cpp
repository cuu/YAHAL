...
// is read in via channel 6 of the ADC.
...
// A timer is used to periodically sample the
// ADC. When 512 samples are taken, the timer
...
//    adc14_rp2040::inst.adcSetupScan(ADC::ADC_14_BIT);
//    adc14_rp2040::inst.adcStartScan(10, 10);

    // Setup timer for measurements
    // T[us] = 1E6[us] / f (result is 125us for f=8kHz)
    timer_rp2040 timer(0);
    timer.setPeriod(1000000/SAMPLE_FREQUENCY, TIMER::PERIODIC);
    timer.setCallback([&]() {
        static int index = 0;
        // Read in ADC value
        uint16_t val = adc_rp2040::inst.adcReadRaw(6);
