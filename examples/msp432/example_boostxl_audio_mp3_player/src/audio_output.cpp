      ...
      _zero(0), _one(BIT2)
{
    ...
    _dma_control_data[0].SRC_DATA_END_PTR = &_tasks[3].unused;
    _dma_control_data[0].DST_DATA_END_PTR = &_dma_control_data[8].unused;
    _dma_control_data[0].CTRL.CYCLE_CTRL  = DMA::CYCLE_MEM_SCATTER_GATHER_PRI;
    _dma_control_data[0].CTRL.N_MINUS_1   = 4 * 4 - 1;
    _dma_control_data[0].CTRL.R_POWER     = DMA::ARB_AFTER_4;
    _dma_control_data[0].CTRL.SRC_SIZE    = DMA::WORD;
    _dma_control_data[0].CTRL.SRC_INC     = DMA::WORD;
    _dma_control_data[0].CTRL.DST_SIZE    = DMA::WORD;
    _dma_control_data[0].CTRL.DST_INC     = DMA::WORD;
    _dma_ctrl_backup = _dma_control_data[0].CTRL;

    DMA_Channel->CH_SRCCFG[0] = 0;
    DMA_Control->CFG          = 1;
    DMA_Control->CTLBASE      = (uint32_t)_dma_control_data;
}
...
void audio_output::handler(void * data) {
    ...
    if (_this->_pcm_fifo.get(_this->_pcm_value)){
        ...
        _this->_dma_control_data[0].CTRL = _this->_dma_ctrl_backup;
        DMA_Control->ENASET    = 1;
