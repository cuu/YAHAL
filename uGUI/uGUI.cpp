...
uGUI::uGUI(lcd_interface & lcd) : _lcd(lcd) {
    x_dim = _lcd.getSizeX();
    y_dim = _lcd.getSizeY();
    console.x_start = 4;
    console.y_start = 4;
    console.x_end = x_dim - console.x_start-1;
    console.y_end = y_dim - console.x_start-1;
    console.x_pos = console.x_end;
    console.y_pos = console.y_end;
    font.char_h_space = 1;
    font.char_v_space = 1;
    font.p = nullptr;
    desktop_color = 0x5E8BEf;
    fore_color = C_WHITE;
    back_color = C_BLACK;
    next_window = nullptr;
...
{
   font.p           = f->p;
   font.char_width  = f->char_width;
   font.char_height = f->char_height;
}

void uGUI::FillScreen( UG_COLOR c )
{
   FillFrame(0, 0, x_dim-1, y_dim-1, c);
}
...
void uGUI::FillFrame( int16_t x1, int16_t y1, int16_t x2, int16_t y2, UG_COLOR c )
{
   ...
   if ( x2 < x1 ) { n = x2; x2 = x1; x1 = n; }
   if ( y2 < y1 ) { n = y2; y2 = y1; y1 = n; }

   _lcd.fillArea(x1, y1, x2, y2, c);
}

void uGUI::FillRoundFrame( int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t r, UG_COLOR c )
{
   int16_t  x,y,xd;
   /* swap coordinates if necessary */
...
void uGUI::DrawRoundFrame( int16_t x1, int16_t y1, int16_t x2, int16_t y2, int16_t r, UG_COLOR c )
{
   int16_t n;
   if ( x2 < x1 )
   {
...
void uGUI::DrawArc( int16_t x0, int16_t y0, int16_t r, uint8_t s, UG_COLOR c )
{
   ...
   {
      // Q1
      if ( s & 0x01 ) _lcd.drawPixel(x0 + x, y0 - y, c);
      if ( s & 0x02 ) _lcd.drawPixel(x0 + y, y0 - x, c);

      // Q2
      if ( s & 0x04 ) _lcd.drawPixel(x0 - y, y0 - x, c);
      if ( s & 0x08 ) _lcd.drawPixel(x0 - x, y0 - y, c);

      // Q3
      if ( s & 0x10 ) _lcd.drawPixel(x0 - x, y0 + y, c);
      if ( s & 0x20 ) _lcd.drawPixel(x0 - y, y0 + x, c);

      // Q4
      if ( s & 0x40 ) _lcd.drawPixel(x0 + y, y0 + x, c);
      if ( s & 0x80 ) _lcd.drawPixel(x0 + x, y0 + y, c);
...
void uGUI::DrawLine( int16_t x1, int16_t y1, int16_t x2, int16_t y2, UG_COLOR c )
{
   ...
   if ( y1 == y2 ) { _lcd.drawHLine(x1, y1, x2, c); return; }
   if ( x1 == x2 ) { _lcd.drawVLine(x1, y1, y2, c); return; }
...
void uGUI::PutString( int16_t x, int16_t y, char* str )
{
   ...
   while ( *str != 0 )
   {
      ...
      if ( xp + font.char_width > x_dim - 1 )
      {
         xp = x;
         yp += font.char_height + font.char_v_space;
      }

      PutChar(chr, xp, yp, fore_color, back_color);

      xp += font.char_width + font.char_h_space;
      str++;
   }
}
...
void uGUI::ConsolePutString( char* str )
{
   ...
   while ( *str != 0 )
   {
      ...
      console.x_pos += font.char_width+font.char_h_space;

      if ( console.x_pos+font.char_width > console.x_end )
      {
         console.x_pos = console.x_start;
         console.y_pos += font.char_height+font.char_v_space;
      }
      if ( console.y_pos+font.char_height > console.y_end )
      {
         console.x_pos = console.x_start;
         console.y_pos = console.y_start;
         FillFrame(console.x_start,console.y_start,console.x_end,console.y_end,console.back_color);
      }

      PutChar(chr, console.x_pos, console.y_pos, console.fore_color, console.back_color);
      str++;
   }
}
...
{
   console.fore_color = c;
}

void uGUI::ConsoleSetBackcolor( UG_COLOR c )
{
   console.back_color = c;
}
...
/* -- INTERNAL FUNCTIONS                                                         -- */
/* -------------------------------------------------------------------------------- */
void uGUI::_PutText(UG_TEXT* txt)
{
   uint16_t sl,rc;
   int16_t xp,yp;
   int16_t xs=txt->a.xs;
...
void uGUI::_DrawObjectFrame( int16_t xs, int16_t ys, int16_t xe, int16_t ye, UG_COLOR* p )
{
   // Frame 0
...
void uGUI::Update( void )
{
   ...
   if ( state & UG_SATUS_WAIT_FOR_UPDATE ) state &= ~UG_SATUS_WAIT_FOR_UPDATE;

   /* Keep track of the windows */
   if ( next_window != active_window )
   {
      if ( next_window != nullptr )
      {
         ...
         if ( (last_window->style & WND_STYLE_SHOW_TITLE) && (last_window->state & WND_STATE_VISIBLE) )
         {
            /* Do both windows differ in size */
            if ( (last_window->xs != active_window->xs) || (last_window->xe != active_window->xe) || (last_window->ys != active_window->ys) || (last_window->ye != active_window->ye) )
            {
               /* Redraw title of the last window */
               _WindowDrawTitle( last_window );
            }
         }
         active_window->state &= ~WND_STATE_REDRAW_TITLE;
         active_window->state |= WND_STATE_UPDATE | WND_STATE_VISIBLE;
      }
   }
...
{
   state |= UG_SATUS_WAIT_FOR_UPDATE;
   while ( (volatile uint8_t)state & UG_SATUS_WAIT_FOR_UPDATE ){};
}

void uGUI::DrawBMP( int16_t xp, int16_t yp, BMP* bmp )
{
   if ( bmp->p == nullptr ) return;

   /* Only support 16 BPP so far */
   if ((bmp->bpp != BMP_BPP_16) || (bmp->colors != BMP_RGB565)) return;
...
/* -- WINDOW FUNCTIONS                                                           -- */
/* -------------------------------------------------------------------------------- */
UG_RESULT uGUI::WindowCreate( WINDOW* wnd, OBJECT* objlst, uint8_t objcnt, void (*cb)( MESSAGE* ) )
{
   uint8_t i;
   OBJECT* obj=nullptr;

   if ( (wnd == nullptr) || (objlst == nullptr) || (objcnt == 0) ) return UG_RESULT_FAIL;
...
{
   if ( wnd == active_window ) return UG_RESULT_FAIL;
...
UG_RESULT uGUI::WindowHide( WINDOW* wnd )
{
   if ( wnd != nullptr )
   {
      if ( wnd == active_window )
      {
         ...
         if ( (last_window != nullptr) && (last_window->state & WND_STATE_VISIBLE) )
         {
            if ( (last_window->xs > wnd->xs) || (last_window->ys > wnd->ys) || (last_window->xe < wnd->xe) || (last_window->ye < wnd->ye) )
            {
               _WindowClear( wnd );
            }
            next_window = last_window;
         }
         else
         {
            active_window->state &= ~WND_STATE_VISIBLE;
            active_window->state |= WND_STATE_UPDATE;
         }
      }
      else
      {
...
UG_RESULT uGUI::WindowResize( WINDOW* wnd, int16_t xs, int16_t ys, int16_t xe, int16_t ye )
{
   int16_t pos;
   int16_t xmax,ymax;
   ...
   if ( (wnd != nullptr) && (wnd->state & WND_STATE_VALID) )
   {
      ...
      if ( (wnd->state & WND_STATE_VISIBLE) && (active_window == wnd) )
      {
         if ( wnd->ys ) FillFrame(0, 0, xmax,wnd->ys-1,desktop_color);
         pos = wnd->ye+1;
         if ( !(pos > ymax) ) FillFrame(0, pos, xmax,ymax,desktop_color);
         if ( wnd->xs ) FillFrame(0, wnd->ys, wnd->xs-1,wnd->ye,desktop_color);
         pos = wnd->xe+1;
         if ( !(pos > xmax) ) FillFrame(pos, wnd->ys,xmax,wnd->ye,desktop_color);
...
UG_RESULT uGUI::WindowAlert( WINDOW* wnd )
{
   ...
   if ( WindowSetTitleTextColor( wnd, WindowGetTitleColor( wnd ) ) == UG_RESULT_FAIL ) return UG_RESULT_FAIL;
   if ( WindowSetTitleColor( wnd, c ) == UG_RESULT_FAIL ) return UG_RESULT_FAIL;
   return UG_RESULT_OK;
}
...
UG_RESULT uGUI::WindowSetXStart( WINDOW* wnd, int16_t xs )
{
   ...
   {
      wnd->xs = xs;
      if ( WindowResize( wnd, wnd->xs, wnd->ys, wnd->xe, wnd->ye) == UG_RESULT_FAIL ) return UG_RESULT_FAIL;
      return UG_RESULT_OK;
   }
   return UG_RESULT_FAIL;
}
...
UG_RESULT uGUI::_WindowDrawTitle( WINDOW* wnd )
{
   ...
   if ( (wnd != nullptr) && (wnd->state & WND_STATE_VALID) )
   {
      ...
      /* Draw line */
      DrawLine(xs,ys+wnd->title.height,xe,ys+wnd->title.height,pal_window[11]);
      return UG_RESULT_OK;
   }
   return UG_RESULT_FAIL;
}
...
void uGUI::_WindowUpdate( WINDOW* wnd )
{
   ...
   {
      FillFrame(wnd->xs,wnd->xs,wnd->xe,wnd->ye,desktop_color);
   }
}
...
UG_RESULT uGUI::_WindowClear( WINDOW* wnd )
{
   if ( wnd != nullptr )
   {
      if (wnd->state & WND_STATE_VISIBLE)
      {
         ...
         FillFrame( wnd->xs, wnd->ys, wnd->xe, wnd->ye, desktop_color );

         if ( wnd != active_window )
         {
            /* If the current window is visible, update it! */
            if ( active_window->state & WND_STATE_VISIBLE )
            {
               active_window->state &= ~WND_STATE_REDRAW_TITLE;
               active_window->state |= WND_STATE_UPDATE;
            }
...
/* -- BUTTON FUNCTIONS                                                           -- */
/* -------------------------------------------------------------------------------- */
UG_RESULT uGUI::ButtonCreate( WINDOW* wnd, BUTTON* btn, uint8_t id, int16_t xs, int16_t ys, int16_t xe, int16_t ye )
{
   OBJECT* obj;

   obj = _GetFreeObject( wnd );
   if ( obj == nullptr ) return UG_RESULT_FAIL;
...
UG_RESULT uGUI::ButtonSetAlternateForeColor( WINDOW* wnd, uint8_t id, UG_COLOR afc )
{
   OBJECT* obj=nullptr;
   BUTTON* btn=nullptr;
...
UG_RESULT uGUI::ButtonSetAlternateBackColor( WINDOW* wnd, uint8_t id, UG_COLOR abc )
{
   OBJECT* obj=nullptr;
   BUTTON* btn=nullptr;
   ...
   if ( obj->state & OBJ_STATE_UPDATE )
   {
      if ( obj->state & OBJ_STATE_VISIBLE )
      {
         ...
         if ( (obj->state & OBJ_STATE_REDRAW) || (btn->state & BTN_STATE_ALWAYS_REDRAW) )
         {
            ...
            gui->FillFrame(obj->a_abs.xs+d, obj->a_abs.ys+d, obj->a_abs.xe-d, obj->a_abs.ye-d, txt.bc);
         ...
         {  /* 3D */
            gui->_DrawObjectFrame(obj->a_abs.xs,obj->a_abs.ys,obj->a_abs.xe,obj->a_abs.ye, (btn->state&BTN_STATE_PRESSED)?(UG_COLOR*)pal_button_pressed:(UG_COLOR*)pal_button_released);
         }
         else
         {  /* 2D */
            gui->DrawFrame(obj->a_abs.xs,obj->a_abs.ys,obj->a_abs.xe,obj->a_abs.ye,(btn->state&BTN_STATE_PRESSED)?btn->abc:btn->afc);
         }
      }
      else
      {
         gui->FillFrame(obj->a_abs.xs, obj->a_abs.ys, obj->a_abs.xe, obj->a_abs.ye, wnd->bc);
      }
      obj->state &= ~OBJ_STATE_UPDATE;
   }
}
...
/* -- TEXTBOX FUNCTIONS                                                          -- */
/* -------------------------------------------------------------------------------- */
UG_RESULT uGUI::TextboxCreate( WINDOW* wnd, TEXTBOX* txb, uint8_t id, int16_t xs, int16_t ys, int16_t xe, int16_t ye )
{
   OBJECT* obj;
   ...
   if ( obj->state & OBJ_STATE_UPDATE )
   {
      if ( obj->state & OBJ_STATE_VISIBLE )
      {
         ...
         if ( obj->state & OBJ_STATE_REDRAW )
         {
            ...
            gui->FillFrame(obj->a_abs.xs, obj->a_abs.ys, obj->a_abs.xe, obj->a_abs.ye, txt.bc);
...
/* -- IMAGE FUNCTIONS                                                            -- */
/* -------------------------------------------------------------------------------- */
UG_RESULT uGUI::ImageCreate( WINDOW* wnd, IMAGE* img, uint8_t id, int16_t xs, int16_t ys, int16_t xe, int16_t ye )
{
   OBJECT* obj;
   ...
   if ( obj->state & OBJ_STATE_UPDATE )
   {
      if ( obj->state & OBJ_STATE_VISIBLE )
      {
         ...
         if ( obj->state & OBJ_STATE_REDRAW )
         {
            ...
            obj->a_abs.ye = obj->a_rel.ys + ((BMP*)img->img)->height + a.ys;
            if ( obj->a_abs.ye >= wnd->ye ) return;
            if ( obj->a_abs.xe >= wnd->xe ) return;
